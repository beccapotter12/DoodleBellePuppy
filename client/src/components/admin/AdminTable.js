...
// Update these components to Bootstrap React components 
...
class AdminTable extends Component {
  getStatus = (status) => {
    return <Status state={status}>{status.charAt(0)}</Status>
  }
  render() {
    const { view_status, orders, statuses, shipping } = this.props;
    return (
      <div>
        <Tabs value={view_status} onChange={this.props.switchView}>
        ...
        <Table>
          <tbody>
            { orders.map((order,i) => {
              ...
              return (
                <tr key={`order${i}`}>
                  <td>{this.getStatus(order.metadata.status)}</td>
                  <td>
                  ...
                  <td>
                    <select value={statuses[i]} 
                      onChange={(e) => this.props.updateStatus(i, e.target.value)}
                    >
                  ...
                  <td>
                    ...
                    { order.shipping.tracking_number ?
                      ...
                      <input type="text" value={shipping[i]}
                        onChange={(e) => this.props.updateShipping(i, e.target.value)}
                      />
                    }
                  </td>
                  <td>
                    <Button onClick={() => this.props.updateOrder(i)}
